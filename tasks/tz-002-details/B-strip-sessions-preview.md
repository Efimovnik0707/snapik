...
**На диске.** Корень `%LOCALAPPDATA%\SnapBrief\sessions` (`SessionWorkspace.cs:32`), рядом на уровень выше `settings.json` и `last-region.json` (`:36`, `:43`). Внутри корня:
...
| # | Файлы |
|---|---|
| B1 | `src/SnapBrief.App/SessionWorkspace.cs`, `src/SnapBrief.App/EdgeStackWindow.xaml.cs`, `src/SnapBrief.App/DiscardSessionWindow.xaml` (новый), `src/SnapBrief.App/DiscardSessionWindow.xaml.cs` (новый), `src/SnapBrief.App/HotkeySettingsWindow.xaml.cs` (только запись `HotkeySettings`), `src/SnapBrief.App/UiLanguage.cs`, `src/SnapBrief.App/SmokeTestRunner.cs`, `tasks/verification.md` |
| B2 | `src/SnapBrief.App/EdgeStackWindow.xaml`, `src/SnapBrief.App/EdgeStackWindow.xaml.cs`, `src/SnapBrief.App/Controls/StripResizeGeometry.cs`, `src/SnapBrief.App/HotkeySettingsWindow.xaml.cs` (только запись `HotkeySettings`), `tests/SnapBrief.App.Imaging.Tests/StripResizeGeometryTests.cs`, `src/SnapBrief.App/SmokeTestRunner.cs`, `tasks/verification.md` |
| B3 | `src/SnapBrief.App/EdgeStackWindow.xaml`, `tasks/verification.md` |
| B4 | `src/SnapBrief.App/EdgeStackWindow.xaml`, `tasks/verification.md` |
| A6 | `src/SnapBrief.Core/Exporting/SentCaptureRules.cs`, `src/SnapBrief.App/EdgeStackWindow.xaml.cs`, `src/SnapBrief.App/UiLanguage.cs`, `tests/SnapBrief.Core.Tests/SessionModelTests.cs`, `src/SnapBrief.App/SmokeTestRunner.cs`, `tasks/verification.md` |
| F1 (лента) | `src/SnapBrief.App/EdgeStackWindow.Saving.cs`, `src/SnapBrief.App/EdgeStackWindow.xaml.cs`, `tasks/verification.md` |
| D1 (лента) | удаляются `src/SnapBrief.App/CapturePreviewWindow.xaml`, `src/SnapBrief.App/CapturePreviewWindow.xaml.cs`, `src/SnapBrief.App/EdgeStackWindow.Preview.cs`; правятся `src/SnapBrief.App/UiLanguage.cs`, `src/SnapBrief.App/EdgeStackWindow.xaml.cs`, `src/SnapBrief.App/SmokeTestRunner.cs`, `tasks/verification.md` |
| E (подготовка) | `src/SnapBrief.App/ThemeService.cs`, `src/SnapBrief.App/Themes/*`, `src/SnapBrief.App/App.xaml`, `src/SnapBrief.App/Interop/SystemBackdrop.cs` (новый), позже `EdgeStackWindow.xaml(.cs)` и все окна с тёмными литералами |
...
2. **Никите, B1 и старые сессии.** Первый запуск новой версии удалит всё, что накопилось в `%LOCALAPPDATA%\SnapBrief\sessions`, включая снимки, которые пользователь мог считать сохранёнными. Молча или один раз с уведомлением? Предлагаю молча: контракт «сессия живёт один запуск» иначе не читается.
