...
11. **A4: «стрелка сдвигает на 116 px» не дефект и ничему не противоречит.** `PageBy` просит `_firstCard * CardStep`, а `ScrollViewer` сам клампит запрос по `ScrollableWidth` (`AppearancePicker.xaml.cs:264-268`), и `OnGalleryScrolled` возвращает счёт в согласие с тем, что на экране. Шаг остаётся 142 на всех ходах, кроме последнего, и последняя карточка при этом видна целиком: требования ТЗ друг другу не мешают. Настоящий дефект другой: галерея открывается прокрученной к выбранной теме (`BringSelectedCardIntoView`, `:243`, `:292-298`) (`A §1.5`).
...
**Core, вид снимка.** Новый `src/Snapik.Core/Models/CaptureKind.cs`:

```csharp
public enum CaptureKind { Region, Fullscreen, Import }
```

В `src/Snapik.Core/Models/CaptureItem.cs` рядом с существующим `Sent`:

```csharp
public CaptureKind Kind { get; init; } = CaptureKind.Region;
// 0 = неизвестно; заполняется только у Fullscreen. Отрицательное значение из чужого файла
// гасится здесь же, поэтому SessionValidation про это поле ничего не знает.
public int MonitorCount { get => field; init => field = Math.Max(0, value); }
```

Поля-`init`, а не параметры позиционного конструктора: старые `session.json` без `kind` и `monitorCount` читаются как `Region` и `0`, `SchemaVersion` остаётся 1. Отдельный конвертер не нужен, `SnapikJson.Options` уже держит `JsonStringEnumConverter(JsonNamingPolicy.CamelCase)`, то есть `Fullscreen` пишется как `"fullscreen"`. `Title` (позиционный параметр записи, существует) получает смысл: имя файла у `Import`, пустая строка у `Region` и `Fullscreen`.
...
Две точки применения: `AnnotationCanvas.cs:743-744` и `WpfExportImageRenderer.cs:124-126`. Обе сегодня получают готовый `Pen` и передают `item.HasOutline ? pen : null`, а `OutlineColorOf` отдаёт `Color?`, то есть перо нужно пересобрать на месте вызова:
...
Толщина и `DashStyle` (тип линии из прошлого раунда) переносятся из входного пера, кисть строится заново и замораживается. Таблица «заливка → контур → внутри» в `D §2.7`; альфа полупрозрачной сводится к `0x40` в трёх местах (`AnnotationCanvas.cs:734` остаётся, `Appearance.cs:371` и `OverlayEditorWindow.xaml:413` приводятся).

**Список инструментов с толщиной.** `HasStroke` (`Appearance.cs:38`) сегодня определён через `HasColor`, а `HasColor` понятием уходит (D1). `HasStroke` остаётся, но получает явный список и не зависит ни от чего:
...
Список тот же, что давало старое определение (`HasColor` минус `Text`), записанный прямо. От него по-прежнему зависят `ThicknessButton.IsEnabled` (`:263`), подпись кнопки (`:264`) и присвоение толщины в `ApplyAppearance` (`:386`); присвоение цвета (`:385`) становится безусловным.
...
**Оконные помощники.** Новый `src/Snapik.App/DwmWindowCorners.cs` с двумя статическими классами (`A §A1`, `§A3`):
...
Общее правило удаления: `TreatWarningsAsErrors` включён на весь репозиторий (`Directory.Build.props:6`), поэтому осиротевшее приватное поле, приватный метод или неиспользуемая локальная переменная роняют сборку, а не предупреждают. Каждая задача, снимающая последнего потребителя поля, удаляет само поле **в том же коммите**; это касается прежде всего W0-3 (`_activeHasOutline`, `PanelPaintsFill` и всё, что читало только их) и D-1 (`HasColor`).
...
| W0-2 | Модель вида снимка: `CaptureKind`, `Kind`, `MonitorCount` в Core, смысл `Title`, `KindTitle` в `PromptGenerator`, проброс трёх значений через `EditorModels.CaptureItem` (`ToCore`, `FromCore`, `DeepClone`, `Snapshot`, `Restore`, `CaptureSnapshot`) и `Resize.cs:176-178` | M | `Core/Models/CaptureKind.cs` (новый), `Core/Models/CaptureItem.cs`, `Core/Exporting/PromptGenerator.cs`, `EditorModels.cs`, `OverlayEditorWindow.Resize.cs` | `tests/Snapik.Core.Tests`: старая сессия без `kind` читается как `Region`/`0`; `Fullscreen` round-trip как `"fullscreen"`; `PromptGenerator` на `Kind=Fullscreen` без заметок даёт «Снимок A — весь экран.», на `Region` с пустым `Title` даёт пустой результат | чтение старых `session.json`, `SchemaVersion` 1, поведение «снимок без заметок в текст не попадает» для `Region`, обрезка снимка не теряет подпись | три теста зелёные, `prompt.md` демо-сессии не изменился для обычных снимков | `windows: a capture knows what kind it is` |
| W0-3 | `HasOutline` → `LegacyHasOutline` в Core, правило чтения в `FromCore`, `OutlineColorOf` и его две точки применения (перо пересобирается на месте вызова, §3), `HasStroke` получает явный список, удаление поля во всех 27 местах (`D §2.3`), удаление четырёх полей настроек (`AnnotationOutline`, `AnnotationShape`, `AnnotationFill`, `AnnotationFillColor`) вместе с их чтением (`OverlayEditorWindow.xaml.cs:82-84`, `ParseAnnotationShape`/`ParseAnnotationFill`/`ParseAnnotationFillColor` вместе с полями, если больше некому звать), записью (`Appearance.cs:672-678`, четыре строки инициализатора) и упоминанием в `customSettings` смоука (`SmokeTestRunner.cs:36-38`), сведение альфы полупрозрачной к `0x40` | L | `Core/Models/AnnotationItem.cs`, `EditorModels.cs`, `Controls/AnnotationCanvas.cs`, `WpfExportImageRenderer.cs`, `OverlayEditorWindow.xaml` (`:310-315`, `:413`), `.xaml.cs` (`:56, 82-85, 355-370, 1008`), `.Appearance.cs` (`:37-38, 120-128, 256, 326-327, 378-392, 471-475, 672-678`), `HotkeySettingsWindow.xaml.cs:65-71`, `SmokeTestRunner.cs:36-38, 425, 1028-1044`, `tests/` | `D §7` пункты 1-3: чтение `hasOutline=false` у прямоугольника, свой `fillColor` сохраняется, `hasOutline=true` и отсутствие поля читаются как `Fill=None`, стрелка с `hasOutline=false` не трогается; записанный 1.5.0 `session.json` не содержит подстроки `hasOutline` (проверять по тексту файла); `VerifyLegacyRedactionReadsAsAFilledRegion` переписан | экспорт PNG (контур и заливка на своих местах у всех четырёх видов заливки), размытие без контура, порядок слоёв (непрозрачная заливка последней), чтение сессий 1.2.x с `redaction`, толщина на кнопке панели у четырёх инструментов со штрихом | `scripts/build.ps1` зелёный, `grep -rn "HasOutline" src tests` даёт только `LegacyHasOutline`, `grep -rn "AnnotationOutline\|PanelPaintsFill" src tests` пуст | `windows: the outline follows the fill instead of a flag` |
| W0-4 | Фикс `LoadBitmap` в обоих местах (C9). Копирование кадра выносится в **новый линкуемый файл** `src/Snapik.App/Imaging/FrameCopy.cs`, `internal static BitmapSource Detach(BitmapSource frame)`: `new WriteableBitmap(frame)` плюс `Freeze()`. Причина выноса: `tests/Snapik.App.Imaging.Tests` не ссылается на проект `Snapik.App` (`ProjectReference` там только на `Snapik.Core`), а подтягивает отдельные файлы через `<Compile Include … Link=…>`; `SessionWorkspace.cs` и `WpfExportImageRenderer.cs` тянут за собой пол-приложения и линковаться не могут, а `Imaging/FrameCopy.cs` не зависит ни от чего, кроме WPF. `SessionWorkspace.LoadBitmap` и `WpfExportImageRenderer.LoadBitmap` зовут хелпер и закрывают поток до возврата | S | `Imaging/FrameCopy.cs` (новый), `SessionWorkspace.cs:274-293`, `WpfExportImageRenderer.cs:51-58`, `tests/Snapik.App.Imaging.Tests/Snapik.App.Imaging.Tests.csproj` (новая строка `<Compile Include>` рядом с `Imaging/RegionBlur.cs`) | `tests/Snapik.App.Imaging.Tests`: `FrameCopy.Detach` на кадре, прочитанном `BitmapDecoder` с настоящего PNG и JPG, даёт замороженную копию тех же размеров; эта копия кодируется в `Task.Run` без `InvalidOperationException` (кадр декодера кидал); smoke дополнительно гоняет весь путь `LoadBitmap` плюс `AddImageAsync` на файле с диска (в задаче C-8) | импорт PNG и JPG, экспорт PNG из `FileExportService` (он и так идёт вне UI-потока), сообщение об ошибке формата, индексированные форматы (GIF, 8-битный PNG: палитра переезжает в копию) | два теста зелёные, обе `LoadBitmap` зовут `FrameCopy.Detach` и больше не возвращают `BitmapFrameDecode`, импорт PNG и JPG с диска добавляет карточку | `windows: an imported frame leaves its decoder behind` |
| W0-5 | Константы `StripResizeGeometry` по §3 плюс `CardWidth`. Вслед за ними правятся все ожидания, записанные числами: `SmokeTestRunner.cs:33` (`StackWidth = 240` меньше нового минимума, заменить на 260), `SmokeTestRunner.cs:829` (`ClampWidth(1600, 1366) != 1356` → `1366`, потому что `EdgeGap` стал 0), и три `InlineData` в тестах: `:133` (`240, 1920, 240` → `244`), `:136` (`5000, 1920, 1910` → `1920`), `:138` (`1600, 1366, 1356` → `1366`) | M | `Controls/StripResizeGeometry.cs`, `SmokeTestRunner.cs:33, 829`, `tests/Snapik.App.Imaging.Tests/StripResizeGeometryTests.cs:133, 136, 138` | `C §3` пункты 1-3: `MinimumWidth - 2*ShadowMargin == MinimumPanelWidth`, `DefaultWidth == MinimumWidth`, `ClampWidth(208, 1920) == 244` и `ClampWidth(400, 1920) == 400`, `CardWidth(244) == 168` | `InlineData`, записанные ссылкой на константу (`:18`, `:115`, `:134`, `:139`, `:141`), подстраиваются сами и трогать их не надо; `WidthFromStart`/`ListHeightFromStart` считают от переданных чисел и от констант не зависят; `VerifyStripIsBoundedByItsMonitor` переходит в зону волны 0 целиком | новые тесты зелёные, старые три перечитаны и обновлены (без этого прогон красный, а не молчаливый), smoke не ловит кламп молча | `windows: the strip keeps its panel at 204` |
| W0-6 | Блок B, темы: удаление `Light.xaml`, переписанный `Glass.xaml`, два токена ползунка во все шесть палитр, `ThemeService.Themes` без `light`, `ThemeNames`/`Backdrop` в `AppearancePicker.xaml.cs` (`:31`, `:215`), комментарии «одна из семи палитр» в шапках шести файлов (`Themes/Palettes/*.xaml:4`) и «under all seven palettes» в `SmokeTestRunner.cs:841`, окно-стенд пробы хрома `SmokeTestRunner.cs:870` (`new Window { … Width = 208 … }` → 244, лента столько не бывает) | M | `Themes/Palettes/*.xaml`, `ThemeService.cs:19`, `Controls/AppearancePicker.xaml.cs:29-33, 213-222`, `SmokeTestRunner.cs:841, 870` | smoke `B §8` п. 1 и 4: `Themes.Count == 6`, `Themes` без `"light"`, `NormalizeTheme("light") == "dark"`; `Apply("glass", …)` даёт `SurfaceBrush` с тремя стопами и `SurfaceBarBrush` с `EndPoint=(1,0)` | совпадение наборов ключей всех палитр (`:184-194`), фолбэк неизвестной темы (`:206-208`), хром ленты по всем палитрам (`:846-862`), проверка `sea` (`:200-205`), градиент в капсуле и поповерах | пять оставшихся тем выглядят как в 1.4.0, «Стекло» это градиент B | `windows: six themes and the glass gradient` |
...
| W0-8 | Блок B, миграция: правило темы в `SettingsMigration`, `CurrentVersion` 1 → 2, **порог у каждого правила по своей версии** (`SoundVolume` → `storedVersion < 1`, `Theme` → `storedVersion < 2`), подключение в `HotkeySettings.Migrate` | S | `SettingsMigration.cs`, `HotkeySettingsWindow.xaml.cs:114-121, 187-196`, `tests/Snapik.App.Imaging.Tests/SettingsMigrationTests.cs` | `light` при версии 0 и 1 → `dark`; `sea` не трогается; `light` при версии 2 не трогается; **громкость 60 при версии 1 остаётся 60** (регресс порога) | звук у тех, кто менял громкость; чтение файла без `SettingsVersion`; запись файла назад один раз | четыре теста зелёные, файл с `"Theme":"light"` после запуска несёт `"dark"` и `SettingsVersion: 2` | `windows: the light theme migrates into the dark one` |
...
| A-3 | A3 | Пересчёт `MaxHeight` и центра в DPI монитора показа по формуле `A §A3`: `MonitorMetrics.Scale` у монитора под курсором, позиция через `SetWindowPos` в физических пикселях, однократный `OnDpiChanged` с флагом `_placed`, `Trace` с монитором, масштабом, `MaxHeight` и высотой | M | `OnboardingWindow.xaml.cs:93-108` | арифметику `UsefulHeight` покрыла W0-10, второй раз её не проверяем. Своя проверка здесь на формулу размещения, тоже чистой функцией: центр в физических пикселях (`work.Left + (work.Width - Width*scale)/2`) при рабочей области второго монитора с отрицательным `Left` и при масштабе 1.25 попадает внутрь этой области, а не на соседний монитор | `WindowStartupLocation="Manual"`, открытие на одном мониторе, кламп на 1366×768 | на смешанном DPI окно открывается на мониторе под курсором целиком, `startup.log` несёт строку `Onboarding placement:` | `windows: the wizard measures the monitor it opens on` |
| A-4 | A4 | Шесть карточек (список ведёт волна 0), `BringSelectedCardIntoView` снимается целиком (`:236-244`, `:292-298`), галерея всегда открывается на первой карточке, рамка на выбранной. Колесо: `PreviewMouseWheel` → `PageBy(±1)` с `e.Handled = true` (иначе колесо уходит в `ScrollViewer` мастера). Тачпад: `WM_MOUSEHWHEEL` через `HwndSource.AddHook`, гейт по `Gallery.IsMouseOver`, хук снимается в `Unloaded` (`A §A4`). Имя одно: обработчики зовут существующий `PageBy(int)`, отдельный `PageByWheel` не заводится (в `A §A4` он предлагался только ради smoke, а smoke может звать `PageBy` напрямую, метод приватный в том же классе) | M | `Controls/AppearancePicker.xaml:82-87`, `.xaml.cs:236-298` | `RunProbe`: текст исключения «Seven cards of 132 must not fit into 520» (`AppearancePicker.xaml.cs:362`) переписан на шесть, вместе с комментарием `AppearancePicker.xaml:82`; `PageBy(1)` двигает `_firstCard` на 1 и упирается в `LastPage`; установка `SelectedTheme` на последнюю тему при `_firstCard == 0` галерею не двигает | шевроны `PageBy`/`MarkChevrons`, гашение на концах (opacity 0.42), выбор карточки мышью, вкладка «Вид» в настройках (контрол один, поведение теперь одинаковое в обоих местах) | колесо и горизонтальный жест листают галерею и не прокручивают окно мастера, последняя карточка видна целиком | `windows: the theme gallery listens to the wheel` |
...
| C-8 | C8 (лента) | `SaveFullscreenAsync` переписывается в `CaptureFullscreenAsync` по хвосту обычного захвата (`C §C8`). Порядок шагов задан жёстко и меняться не должен: `await _pasteIntentTransition` → `CaptureIsBlockedByADialog("fullscreen capture")` → `_busy` → **`StripIsFull()`** → `HideForCapture()` → `Task.Delay(120)` → `CaptureDesktopFrame` → `AddImageAsync` → `Kind = Fullscreen`, `MonitorCount = Screen.AllScreens.Length` → `Captures.Add` → `Renumber`/`NoteStripGrowth`/`InvalidatePrepared` → звук → `SaveAndCopyCommittedPackageAsync` → `AutoSaveCaptureAsync`, в `finally` `_busy = false` и `ShowStackWithoutActivation()`. `StripIsFull()` стоит **до** `HideForCapture()`, как в обычном захвате (`EdgeStackWindow.xaml.cs:550`, комментарий там же объясняет почему): сегодня у `SaveFullscreenAsync` этой проверки нет вовсе (`EdgeStackWindow.Saving.cs:43-45`), потому что снимок шёл мимо ленты, а теперь он в ленту идёт, и нажатие на полной ленте не должно гасить все окна ради снимка, которому некуда лечь. Прямая запись в папку уходит. Чип «экран» справа в полосе подписи по `DataTrigger Binding="{Binding Kind}" Value="Fullscreen"`, глиф `TVMonitor` `E7F4` из `IconFont`; тем же триггером миниатюра переходит на `Stretch="Uniform"`. Импорт заполняет `Title` именем файла и `Kind = Import`, его чип глиф `Page` `E7C3`. Заодно иконки ленты по таблице `D §3.3` переводятся на `IconFont` (корзина `E74D`, три точки `E712`, свернуть `E921`, закрыть `E8BB`, комментарий `E90A`, камера `E722`); стрелка вверх (`:316`) не трогается, имя глифа не подтверждено | L | `EdgeStackWindow.Saving.cs:39-57`, `EdgeStackWindow.xaml:150-161, 247-260, 301, 344`, `.xaml.cs:284, 1060` | smoke: снимок с `Kind = Fullscreen`, `MonitorCount = 2` проходит `PrepareAsync` и даёт в `prompt.md` «Снимок A — весь экран.»; `SessionWorkspace.LoadBitmap` плюс `AddImageAsync` на настоящем файле с диска | id клавиши `fullscreen-save` и поле `FullscreenSaveId` (формат), автосохранение, Ctrl+V пакет, `StripIsFull`, всплывашка трея | снимок всего экрана ложится в ленту карточкой с чипом и уходит в Ctrl+V; импорт даёт свой чип и подпись; **27-е нажатие клавиши на полной ленте даёт тост о лимите и не прячет окна** (экран не мигает) | `windows: the whole screen lands in the strip` |
...
| # | Пункты ТЗ | Решение | Размер | Файлы | Тесты и smoke | Что не должно сломаться | Критерий готовности | Коммит |
|---|---|---|---|---|---|---|---|---|
| D-1 | D1 | Один активный цвет (`D §2.1`): понятие `HasColor` уходит, цвет принимается при любом инструменте, `AppearanceButton.IsEnabled` (`:261`) и `ColorDots.IsEnabled` (`:334`) всегда `true`, `ApplyQuickColor` сводится к обычному присвоению. `PanelPaintsFill` и `mainColor` в этой задаче уже не трогаются: `PanelPaintsFill` читал `HasOutline` и погиб вместе с ним в W0-3, где `mainColor` стал просто цветом (то есть половина «кружки красят активный цвет» приезжает из волны 0, и это нормально). Заливка остаётся свойством фигуры со своим цветом (`D §2.2`); `_activeShape`, `_activeFill`, `_activeFillColor` больше не читаются из настроек (поля удалены в волне 0), каждый снимок начинается с «контур, без заливки, прямоугольник». Спектр и пипетка видны в каждой палитре, автозапоминание цвета снимается, вместо него кнопка «+» 34×34 третьей колонкой рядом с HEX, `RememberCustomColor` перестаёт выходить при чужой активной палитре (`D §2.6`) | L | `OverlayEditorWindow.Appearance.cs:37, 260-262, 318-341, 363, 384, 425-429, 471-506, 549-558`, `.xaml:296-309`, `.xaml.cs:355-370` | `PanelChecks`: блок `:355-370` волна 0 довела до компилируемого минимума (выкинув проверки переключателя рамки и «точка красит заливку»), а четыре новые проверки пишет эта задача (цвет, выбранный при инструменте «Комментарий», доезжает до `Surface.ActiveColor` и достаётся следующей рамке; кружок панели показывает активный цвет, а не заливку; спектр и пипетка видны при каждой из трёх палитр; «+» кладёт активный цвет в ряд «Своей»); `RunCustomPaletteProbe` переписан | запоминание между снимками цвета, толщины, палитры и карандаша/маркера; ряд из 12 слотов «Своей» и его переживание перезапуска; HEX в обе стороны; выделенная фигура меняет цвет контура, не теряя цвет заливки | выбрать цвет при «Комментарии», потом рамку: рамка того цвета; три рамки (без заливки, сплошная, размытие) выглядят как в таблице `D §2.7` | `windows: one active colour for every tool` |
| D-2 | D3 | Порядок нажатия при активном «Комментарии» (`D §4.1`), четыре правки в `AnnotationCanvas`: **(1)** `FindLeaderAnchor` (`:773`) меняет условие `Tool != EditorTool.Select` на `Tool is not (EditorTool.Select or EditorTool.Comment)`, а не снимает его вовсе: комментарий `:769-770` объясняет, зачем оно стоит (с рамкой, стрелкой, пером или подписью в руке нажатие в семи пикселях от булавки обязано рисовать, а не тащить), и снятое целиком оно отнимет рисование у четырёх инструментов. **(2)** `FindMoveHandle` (`:786`) теряет `Tool == EditorTool.Comment`, вместо этого `IsMoveHandle` при `Tool == Comment` отвечает только за отметки `Kind == Comment` (иначе булавку нельзя поставить поверх нарисованной рамки). **(3)** Ворота `:225` теряют `Tool != EditorTool.Comment`, и там же `handleHit` из `FindResizeHandle` при `Tool == Comment` гасится: метод отдаёт углы **выделенной** отметки при любом инструменте (`:544-551`), поэтому без гашения сценарий «выделил рамку, взял Комментарий, нажал рядом с её углом» потянет угол вместо новой булавки. **(4)** Курсор: `movablePin` (`:358`) получает то же `Tool is EditorTool.Select or EditorTool.Comment`, и гашение углов из пункта 3 обязано действовать и здесь (`:356-357`), иначе курсор над углом пообещает изменение размера, которого нажатие не сделает; проще всего сделать ранний выход внутри самого `FindResizeHandle`, тогда обе точки чинятся одной строкой. Плюс вариант Б для остальных инструментов (`D §4.3`, решение 5): при `item.Kind == Tool` полоса захвата расширяется с 6 до 10 px, нутро пустой рамки остаётся свободным | M | `Controls/AnnotationCanvas.cs:213-228, 341-362, 544-551, 771-788, 790-827` | `VerifyGestureRules` дополняется тремя сценариями D3: при инструменте «Комментарий» нажатие точно по бейджу не создаёт второй отметки, выделяет первую, и `Tool` остаётся `Comment`; то же для якоря (у отметки должен быть `Label`); **выделенная рамка плюс инструмент «Комментарий», нажатие в углу её габаритов ставит булавку, а не тянет угол**. `VerifyHoverManipulation`: при `Tool == Comment` `FindMoveHandle` над кромкой рамки даёт `null`, над бейджем даёт комментарий; при `Tool == Rectangle` нажатие в семи пикселях от булавки заводит черновик, а не тащит якорь | ластик, двойной клик (открывает заметку), изменение размера рамки при «Выборе» и при «Области», рисование рамки внутри рамки, захват стрелки за линию и пера за штрих, пилюля (её нажатие перехватывает `ChipLayer` и до холста не доходит) | два комментария подряд, первый двигается за бейдж и за якорь без переключения инструмента; рамка по-прежнему тянется за углы, когда инструмент не «Комментарий» | `windows: the comment tool grabs what is already there` |
...
| W2-3 | Версия 1.4.0 → 1.5.0 и сборка установщика | S | `src/Snapik.App/Snapik.App.csproj`, `installer/` | `artifacts/installer/Snapik-Setup-1.5.0.exe` собран |
...
Порядок слияния: **A → C → D**, по одному, после каждого `scripts/build.ps1 -OutputDirectory $env:LOCALAPPDATA\Temp\snapik-candidate-tz005` зелёный. Обоснование порядка: C читает `HotkeySettingsWindow.OnboardingRequested`, которое ставит A (само свойство объявлено в волне 0, поэтому ветки собираются в любом порядке, но осмысленный результат даёт только A перед C); C несёт самую крупную структурную переделку, и чем раньше она в `master`, тем больше прогонов проходит поверх неё; D замкнута в файлах редактора и ложится последней без риска для ленты.

**Пересчёт адресов после волны 0.** Волна 0 заходит в восемь файлов, которые дальше принадлежат дорожкам: `OverlayEditorWindow.xaml`, `OverlayEditorWindow.xaml.cs`, `OverlayEditorWindow.Appearance.cs`, `Controls/AnnotationCanvas.cs` (удаление `HasOutline` и `OutlineColorOf`), `SessionWorkspace.cs` и `WpfExportImageRenderer.cs` (C9), `Controls/StripResizeGeometry.cs` (константы), `OnboardingWindow.xaml.cs` (`UsefulHeight`), плюс `App.xaml` и `SettingsMigration.cs`. Все `file:line` в разделах 3 и 4 даны по `a2f72e1`; после волны 0 они сдвигаются. Первый шаг каждого исполнителя волны 1: пересчитать адреса своих правок по коммиту волны 0 (`search_graph` по имени, потом grep), а не идти по номерам строк вслепую.

| Файл | Кто пишет | Правило |
|---|---|---|
| `UiLanguage.cs` | только волна 0 | В волне 1 файл закрыт. Если дорожка обнаружит пропущенную пару, она дописывает её **в конец таблицы** (не рядом с родственными: конец даёт три соседние строки вместо трёх врезок в разные места) и называет пару в своих заметках. После каждого merge: `grep -o '\["[^"]*"\]' src/Snapik.App/UiLanguage.cs \| sort \| uniq -d` пуст, и смоук-инвариант уникальности английских значений (`SmokeTestRunner.cs:1020-1022`) зелёный |
| `SmokeTestRunner.cs` | все | Новые проверки только **отдельными методами в конце файла**, без врезок в чужие блоки. Зоны существующих методов: волна 0 правит `:33`, `:36-38`, `:174`, `:179-221`, `:336-354`, `:425`, `:841`, `VerifyLegacyRedactionReadsAsAFilledRegion` и **обе пробы ленты целиком**, `VerifyStripIsBoundedByItsMonitor` (`:829`, кламп ширины) и `VerifyTheStripChromeFollowsTheTheme` (`:870`, окно-стенд 208); A владеет `RunOnboardingProbe`, `VerifyWizardTranslations`, `VerifySlideKeysStayInsideTheWizard`, `VerifyHowToOnlyWizard`; C после волны 0 в эти две пробы не заходит и пишет только свои новые методы; D владеет пробами редактора (`PanelChecks` живёт в `OverlayEditorWindow.xaml.cs`, это её собственный файл, и волна 0 доводит его блок `:355-370` до компилируемого минимума, а содержательные проверки пишет D-1). Единственная общая точка это список вызовов в `Run`: три соседние строки, конфликт разводится глазами |
...
| `WpfExportImageRenderer.cs` | волна 0, затем D | Волна 0 правит `LoadBitmap` (через `FrameCopy.Detach`) и точку применения `OutlineColorOf`, дальше файл у D |
| `Imaging/FrameCopy.cs`, `SessionWorkspace.cs` | только волна 0 | В волне 1 не трогает никто |
| `Controls/StripResizeGeometry.cs` | только волна 0 | Дорожка C константы читает, но не правит |
| `Themes/**`, `App.xaml`, `SettingsMigration.cs` | только волна 0 | Дорожки палитры, акценты, ресурс шрифта и миграцию не трогают |
| `tests/` | все | Новые тесты только отдельными файлами или отдельными методами в конце существующего класса. Существующие ожидания, записанные числами, правит только волна 0 (W0-5, три `InlineData`) |
...
1. Дубли ключей `UiLanguage`: `grep -o '\["[^"]*"\]' src/Snapik.App/UiLanguage.cs | sort | uniq -d`.
2. Дубли английских значений: держит смоук, но после merge прогнать его первым же.
3. Дубли ключей ресурсов внутри одного словаря: `grep -rho 'x:Key="[^"]*"' src/Snapik.App/Themes/Palettes/*.xaml | sort | uniq -d` по каждому файлу отдельно; между словарём палитры и `SnapikTheme.xaml` дубль молча перекрывает тему, это ловит существующая проверка совпадения наборов ключей.
...
5. Одноимённые методы с разными сигнатурами в partial-классах `OverlayEditorWindow.*.cs`: `grep -rhoP 'private (static )?(async )?[\w<>?\[\], ]+ \K\w+(?=\()' src/Snapik.App/OverlayEditorWindow*.cs | sort | uniq -d` (перегрузки собираются, но два обработчика одного жеста в разных файлах дают двойную обработку).
...
**Автоматически, перед каждым коммитом:** `scripts/build.ps1` (238 тестов плюс `--smoke-test`). Перед каждым слиянием и после него: `scripts/build.ps1 -OutputDirectory $env:LOCALAPPDATA\Temp\snapik-candidate-tz005`.
...
**Для передаточной записки Кате: два числа из ТЗ, которые мы меняем сознательно.** Оба не вопрос, а уведомление: решение принято, обоснование в §2, но в письме они должны стоять отдельно, иначе на приёмке это прочитается как «не сделали».

- **Ширина окна ленты 244 вместо 224** (решение 1). Видимая панель остаётся 204 и карточка 168, ровно как подписано на `reference-png/04`; лишние 20 px это прозрачное поле под тень, которое на экране не видно. Число 224 из C4 посчитано при старом поле 10 и с требованием C6 (поле под тень не меньше радиуса размытия плюс сдвиг) арифметически несовместимо.
- **Высота блока подписей на шаге 5 равна 140, а не 120** (решение 6). При 120 обрезается слайд 4: его вторая строка переносится на две и в русском, и в английском, и блок занимает около 136 px. При 140 запас до появления скроллбара остаётся 3 px, поэтому больше 140 брать нельзя, а точки-листалка стоят на месте на всех четырёх слайдах, как и просит ТЗ.
...
5. **Сохранённая ширина ленты поднимется у всех** (следствие решения 1). У тех, кто тянул ленту руками до 208-243, `ClampWidth` поднимет её до 244 при первом же чтении настроек, и обратно она не опустится. Альтернатива развилке целиком: оставить окно 224 и сократить тень до `BlurRadius 16 / ShadowDepth 3`, тогда хватит поля 10, но тень станет заметно жёстче.
