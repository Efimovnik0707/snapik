---
version: 1
slug: "src-snapik-app-overlayeditorwindow-xaml"
primary_target: "src/Snapik.App/OverlayEditorWindow.xaml"
related_targets: ["src/Snapik.App/EdgeStackWindow.xaml","src/Snapik.App/MainWindow.xaml"]
---

# Snapik capture and stack
...
- Global capture hides all Snapik surfaces. The stack and paste completion must not steal foreground from the user's target composer.
...
Folding saves the capture and prepares the full ordered stack in the clipboard. Ordinary Ctrl+V or the terminal's own image-paste key performs delivery; Snapik does not require a receiver picker or intercept Alt+V in its primary flow. Show clipboard errors on the recoverable stack. A copied single screenshot exposes native image plus file/text formats; multiple screenshots expose the complete file/text package. Receiving all separate images and notes remains an explicit compatibility acceptance condition, not something inferred from successful clipboard writes.
