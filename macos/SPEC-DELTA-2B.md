...
## B. Core-модель (`Sources/SnapBriefCore`)
| Файл | Изменение |
|---|---|
...
Тесты: `Tests/SnapBriefCoreTests/ExtendedCommentsTests.swift` — порт `ExtendedCommentsTests.cs` (метки после Z; 300 заметок round-trip через `SnapBriefJson.encoder/decoder` с `parentAnnotationId`+`arrowStyle="curved"`, prompt содержит `"A301: Комментарий 299 (к области A1)"`; crop снимает связь, заметка "Keep" остаётся).

## C. Редактор (`Sources/SnapBriefMac/Editor` + `Imaging`)
...
## D. Окно предпросмотра (`Sources/SnapBriefMac/Preview/`, новая папка)
...
- `CapturePreviewProbe.swift`: `run(image:) throws` — пустой «+», метка после ввода = последняя из `forNotedAnnotations`, удаление сдвигает метки, 300 записей, `previewBounds(workArea: (-1920,0,1920,1080))` внутри области. XCTest `Tests/SnapBriefMacTests/Preview/PreviewModelTests.swift`, `PreviewGeometryTests.swift`.
...
- Mac: `Tests/SnapBriefMacTests/Preview/PreviewGeometryTests.swift`, `PreviewModelTests.swift`; `Tests/SnapBriefMacTests/Editor/EditorGeometryChipTests.swift` (`findChipPlacement` без пересечений, `findMoveEdge`); `Tests/SnapBriefMacTests/Imaging/ArrowDrawingTests.swift` (4 стиля в 200×100 → непрозрачные пиксели у конца; curved ≠ straight); `Tests/SnapBriefMacTests/App/CaptureFeedbackSoundTests.swift` (`verifyWaveHeaders` не бросает; `HotkeySettings` decode без новых ключей → дефолты).
...
|---|---|---|
| core-shell | `Sources/SnapBriefCore/{Models,Editing,Exporting,Settings}`, `Tests/SnapBriefCoreTests/ExtendedCommentsTests.swift`, `Sources/SnapBriefMac/App/**` (кроме файлов транспорта: `AppCoordinator+PasteIntent.swift`, `PasteInterceptPredicate.swift`, `AsyncGate.swift`), `Sources/SnapBriefMac/Settings/**`, `Sources/SnapBriefMac/Resources/**`, `Tests/SnapBriefMacTests/App/**` (кроме `PasteInterceptPredicateTests.swift`), `Package.swift`, `project.yml`, `CONTRACTS.md` | B, E2–E4, интеграция D в `AppCoordinator.openCapture`, smoke, demo-сид |
| transport | `Sources/SnapBriefCore/Transport/**`, `Tests/SnapBriefCoreTests/Transport/**`, `Sources/SnapBriefMac/Transport/**`, `Tests/SnapBriefMacTests/Transport/**`, новые `App/AppCoordinator+PasteIntent.swift`, `App/PasteInterceptPredicate.swift`, `App/AsyncGate.swift`, `Tests/SnapBriefMacTests/App/PasteInterceptPredicateTests.swift`; правки `App/AppCoordinator.swift` и `+Package.swift` только в точках §4 (согласовано с core-shell: transport делает эти правки, core-shell не трогает `handlePasteIntent`/`completePasteIntent`/`ensureCurrentCaptureSession`/`startNewSession`) | A |
| editor | `Sources/SnapBriefMac/Editor/**`, `Sources/SnapBriefMac/Imaging/**`, `Tests/SnapBriefMacTests/{Editor,Imaging}/**` | C |
| preview | `Sources/SnapBriefMac/Preview/**`, `Sources/SnapBriefMac/Stack/**`, `Tests/SnapBriefMacTests/Preview/**` | D, E1 |
