...
/// Port of `tests/SnapBrief.Core.Tests/PersistenceAndExportTests.cs`.
final class PersistenceAndExportTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        root = FileManager.default.temporaryDirectory.appendingPathComponent("snapbrief-tests-\(SBGuid().digitsLowercase)", isDirectory: true)
    }
    ...
    func test_Json_store_round_trips_the_validated_session() async throws {
        ...
        let session = try SessionOperations.addCapture(
            SnapBriefSession.create(nowUtc: Self.start), capture: capture, nowUtc: Self.start.addingTimeInterval(1))
    ...
    func test_Json_store_loads_schema_one_annotations_without_path_segments() async throws {
        ...
        let session = try SessionOperations.addCapture(SnapBriefSession.create(nowUtc: Self.start), capture: capture, nowUtc: Self.start)
    ...
    func test_Overlapping_saves_commit_the_latest_invocation_even_when_revisions_repeat() async throws {
        ...
        let baseline = try SessionOperations.addCapture(SnapBriefSession.create(nowUtc: Self.start), capture: capture, nowUtc: Self.start)
    ...
    func test_Export_is_an_immutable_ordered_revision_with_hashes_and_no_sources() async throws {
        ...
        var session = try SessionOperations.addCapture(SnapBriefSession.create(nowUtc: Self.start), capture: first, nowUtc: Self.start)
    ...
    func test_Invalid_renderer_output_does_not_publish_a_partial_revision() async throws {
        ...
        } catch let error as SnapBriefError {
    ...
    func test_Whitespace_only_notes_are_not_labeled_exported_or_counted_but_real_text_is_preserved() async throws {
        ...
        var session = try SessionOperations.addCapture(SnapBriefSession.create(nowUtc: Self.start), capture: capture, nowUtc: Self.start)
        session.globalNote = "\r\n\t"
