...
/// `SessionValidation.validate` (`Sources/SnapBriefCore/Models/SessionValidation.swift`), each
...
final class SessionValidationTests: XCTestCase {
    ...
    private func validSession(captures: [CaptureItem] = []) -> SnapBriefSession {
        var session = SnapBriefSession.create(nowUtc: Self.now)
        session.captures = captures
        return session
    }

    private func assertRejected(_ session: SnapBriefSession, contains fragment: String, file: StaticString = #filePath, line: UInt = #line) {
        XCTAssertThrowsError(try SessionValidation.validate(session), file: file, line: line) { error in
            guard case SnapBriefError.invalidData(let message) = error else {
                XCTFail("expected SnapBriefError.invalidData, got \(error)", file: file, line: line)
                return
            }
    ...
    func test_unsupportedSchemaVersionIsRejected() {
        var session = validSession()
        session.schemaVersion = SnapBriefSession.currentSchemaVersion + 1
