...
final class TransportMacTests: XCTestCase {
    ...
    func testMacClipboardServiceWritesExpectedFormatsForSingleAndMultiImagePackages() throws {
        let pasteboard = try XCTUnwrap(NSPasteboard(name: .init("snapik-test-transport")))
    ...
    func testMacClipboardServiceOperationsRunOnADedicatedSerializedQueue() throws {
        let pasteboard = try XCTUnwrap(NSPasteboard(name: .init("snapik-test-transport-queue")))
        let dedicatedQueue = DispatchQueue(label: "live.yesworkflow.snapik.tests.clipboard")
    ...
    func testSetPNGGuardedWritesPngTiffAndFileURLInSingleItem() throws {
        let pasteboard = try XCTUnwrap(NSPasteboard(name: .init("snapik-test-transport-pngguarded")))
    ...
    func testCaptureFilePathsExcludesNonFileURLs() throws {
        let pasteboard = try XCTUnwrap(NSPasteboard(name: .init("snapik-test-transport-nonfileurl")))
        let service = MacClipboardService(pasteboard: pasteboard, queue: .main)
        let pngData = try Self.makeSinglePixelPNG()
    ...
    func testCaptureReportsFilesTextAndImageForEchoDetector() throws {
        let pasteboard = try XCTUnwrap(NSPasteboard(name: .init("snapik-test-transport-echo")))
        let service = MacClipboardService(pasteboard: pasteboard, queue: .main)
        let pngData = try Self.makeSinglePixelPNG()
    ...
    private static func writeTempFile(data: Data, name: String) throws -> String {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("SnapikMacTests-\(UUID().uuidString)", isDirectory: true)
