// Port of `src/Snapik.App/CaptureFeedbackSound.cs`, SPEC-DELTA-2.md §1.6, SPEC-DELTA-2B.md §E2.
import AppKit
import Foundation
import SnapikCore
...
enum CaptureFeedbackSound {
    ...
    private static func verifyWave(dataFor name: String) throws {
        guard let url = resourceURL(named: name), let data = try? Data(contentsOf: url) else {
            throw SnapikError.invalidData("Bundled capture feedback WAV \"\(name).wav\" is missing.")
        }
        try verifyWaveHeader(data)
    }

    private static func verifyWaveHeader(_ data: Data) throws {
        let invalid = SnapikError.invalidData(
