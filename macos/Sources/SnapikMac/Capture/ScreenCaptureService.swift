...
public final class ScreenCaptureService: ScreenCaptureServicing {
    private let log = Logger(subsystem: "live.yesworkflow.snapik", category: "capture")
    ...
    private func captureDesktopAsync(includeCursor: Bool) async throws -> DesktopFrame {
        ...
        guard !screens.isEmpty else {
            throw SnapikError.invalidOperation("ScreenCaptureService: no screens are attached.")
        }
        ...
        guard
            ...
            throw SnapikError.invalidOperation("ScreenCaptureService: failed to create the desktop bitmap.")
        }
        ...
        guard let image = composed.makeImage() else {
            throw SnapikError.invalidOperation("ScreenCaptureService: failed to rasterize the desktop image.")
        }
