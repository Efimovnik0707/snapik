// Port of `src/SnapBrief.App/WpfExportImageRenderer.cs:17-150`, SPEC §4.5.
import AppKit
import CoreGraphics
import Foundation
...
public final class ExportImageRenderer: ExportImageRendering {
    ...
    public func renderPNG(
        ...
        guard
            ...
            throw SnapBriefError.invalidOperation("ExportImageRenderer: failed to create the render context.")
        }
        ...
        guard let rendered = ctx.makeImage() else {
            throw SnapBriefError.invalidOperation("ExportImageRenderer: failed to rasterize the export image.")
        }
        guard let data = ImageCodec.encode(rendered, format: .png) else {
            throw SnapBriefError.invalidOperation("ExportImageRenderer: failed to encode the export PNG.")
        }
        return data
    }
    ...
    private func drawCaptureBadge(_ label: String, in ctx: CGContext) {
        ...
        AnnotationPainter.drawText(
            "SNAPBRIEF · СНИМОК", fontSize: 12, weight: .semibold,
