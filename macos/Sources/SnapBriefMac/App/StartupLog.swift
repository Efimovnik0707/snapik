// Port of `StartupTrace` (`src/SnapBrief.App/App.xaml.cs:88-103`), SPEC §1.19.
import Foundation
...
enum StartupLog {
    /// `ISO8601Precise` (Core) is `internal` to `SnapBriefCore`, so the log line timestamp here
    ...
    static func dataRoot(for options: CommandLineOptions) -> URL {
        ...
        return SnapBriefPaths.defaultSessionsDirectory()
    }
