// Port of `src/SnapBrief.App/SessionWorkspace.cs`, SPEC §3.6.
//
// Deviation from the C# structure: on Windows the capture list lives in a separate WPF
// `ObservableCollection` and gets folded into a fresh `SnapBriefSession` snapshot (with a
...
final class SessionWorkspace {
    ...
    private(set) var session: SnapBriefSession
    ...
    init(dataDirectory: URL?, timeProvider: TimeProvider = SystemTimeProvider()) {
        ...
        if let dataDirectory {
            ...
            self.sessionsRoot = SnapBriefPaths.defaultSessionsDirectory()
        ...
        self.session = SnapBriefSession.create(nowUtc: timeProvider.utcNow())
    }
    ...
    func startNewSession() async throws {
        try await save()
        session = SnapBriefSession.create(nowUtc: timeProvider.utcNow())
        try await save()
    }
