...
enum AutoSaveService {
    static func save(
        ...
        guard let sourceImage = ImageCodec.loadImage(at: sourceURL) else {
            throw SnapBriefError.fileNotFound("AutoSaveService: could not load the source image at \(sourceURL.path).")
        }
        ...
        guard
            ...
            throw SnapBriefError.invalidOperation("AutoSaveService: failed to create the render context.")
        }
        ...
        guard let rendered = ctx.makeImage() else {
            throw SnapBriefError.invalidOperation("AutoSaveService: failed to rasterize the annotated image.")
        }
        ...
        guard let data = ImageCodec.encode(rendered, format: format) else {
            throw SnapBriefError.invalidData("AutoSaveService: could not encode the annotated image.")
        }
