...
/// Port of `src/SnapBrief.Core/Models/SessionValidation.cs`.
public enum SessionValidation {
    public static func validate(_ session: SnapBriefSession) throws {
        if session.id == SBGuid.empty {
            throw SnapBriefError.invalidData("Session ID cannot be empty.")
        }
        if session.schemaVersion != SnapBriefSession.currentSchemaVersion {
            throw SnapBriefError.invalidData("Unsupported session schema version: \(session.schemaVersion).")
        }
        if session.revision < 0 {
            throw SnapBriefError.invalidData("Session revision cannot be negative.")
        }
        ...
        for capture in session.captures {
            if capture.id == SBGuid.empty || captureIds.contains(capture.id) {
                throw SnapBriefError.invalidData("Capture IDs must be non-empty and unique.")
            }
            captureIds.insert(capture.id)

            if !RelativePathValidation.isRelativeAndSafe(capture.sourceImagePath) {
                throw SnapBriefError.invalidData(
            ...
            if capture.pixelWidth <= 0 || capture.pixelHeight <= 0 || capture.dpiX <= 0 || capture.dpiY <= 0 {
                throw SnapBriefError.invalidData("Capture dimensions and DPI must be positive.")
            }
            ...
            for annotation in capture.annotations {
                ...
                if annotation.thickness <= 0 || annotation.points.isEmpty {
                    throw SnapBriefError.invalidData("Annotations require positive thickness and geometry.")
                }
