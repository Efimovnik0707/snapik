...
/// Pure-Swift SHA-256 (FIPS 180-4), used in place of `System.Security.Cryptography.SHA256`
/// (`SHA256.HashDataAsync`, see `src/SnapBrief.Infrastructure/Exporting/FileExportService.cs`).
/// No CryptoKit/third-party dependency: CryptoKit is Apple-only, and the Core target must build
/// on Windows Swift, so the algorithm is implemented directly against `Foundation`-only types.
public enum SHA256 {
    ...
    /// Port of `Convert.ToHexStringLower(hash)`.
