...
/// Port of the `HotkeyModifiers` flags enum from `src/SnapBrief.Windows/TransportContracts.cs`.
/// Kept here (not in `SnapBrief.Windows`, which is Win32-only and out of Core's scope) purely as
...
/// Port of `HotkeyChoice` (`src/SnapBrief.App/HotkeySettingsWindow.xaml.cs`), minus the
...
/// Port of `HotkeySettings` (`src/SnapBrief.App/HotkeySettingsWindow.xaml.cs`).
///
/// JSON persistence: the C# `Load`/`Save` use a *default* `JsonSerializerOptions`
/// (`WriteIndented = true` only — no camelCase naming policy, unlike `SnapBriefJson.Options`), so
...
public struct HotkeySettings: Codable, Equatable, Sendable {
    ...
    /// directory (falls back to `~/Pictures/SnapBrief` if unavailable).
    public static func defaultSaveDirectory() -> String {
        ...
        return pictures.appendingPathComponent("SnapBrief", isDirectory: true).path
    }
