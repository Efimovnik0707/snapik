...
/// from `src/SnapBrief.Core/Editing/CaptureCropper.cs`.
...
/// Port of `src/SnapBrief.Core/Editing/CaptureCropper.cs`.
public enum CaptureCropper {
    ...
    private static func validateCrop(
        ...
        if sourcePath.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            ...
            throw SnapBriefError.argument("sourcePath must not be empty or whitespace.")
        }
        if !RelativePathValidation.isRelativeAndSafe(sourcePath) {
            throw SnapBriefError.argument(
                "sourcePath: Cropped source image path must remain relative to the session directory.")
        }
        if pixelWidth <= 0 {
            throw SnapBriefError.argumentOutOfRange("pixelWidth")
        }
        if pixelHeight <= 0 {
            throw SnapBriefError.argumentOutOfRange("pixelHeight")
        }
    }
}
