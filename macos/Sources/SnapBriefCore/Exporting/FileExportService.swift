...
/// Port of `src/SnapBrief.Infrastructure/Exporting/FileExportService.cs`.
///
...
public final class FileExportService: ExportService {
    ...
    public func prepare(session: SnapBriefSession, sessionDirectory: URL) async throws -> PreparedExport {
        try SessionValidation.validate(session)
        guard !session.captures.isEmpty else {
            throw SnapBriefError.invalidOperation("Cannot export a session without captures.")
        }
        ...
        do {
            ...
            for (index, capture) in session.captures.enumerated() {
                ...
                guard FileManager.default.fileExists(atPath: sourceImagePath.path) else {
                    throw SnapBriefError.fileNotFound(
            ...
            let manifestData = try SnapBriefJson.encoder.encode(manifest)
    ...
    private static func countNotes(_ session: SnapBriefSession) -> Int {
    ...
    private static func resolveSessionPath(sessionRoot: URL, relativePath: String) throws -> URL {
        ...
        guard resolved.path.lowercased().hasPrefix(rootPrefix.lowercased()) else {
            throw SnapBriefError.invalidData("Capture image path escapes the session directory.")
        }
        return resolved
    }
    ...
    private static func validatePNG(_ data: Data) throws {
        guard data.count >= pngSignature.count, Array(data.prefix(pngSignature.count)) == pngSignature else {
            throw SnapBriefError.invalidData("The export renderer did not produce a PNG image.")
        }
    }
}
