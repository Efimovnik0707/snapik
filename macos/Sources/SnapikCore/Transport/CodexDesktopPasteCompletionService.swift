...
public final class CodexDesktopPasteCompletionService {
    ...
    public func complete(
        ...
        if intent.clipboardSequence != ownedPackageReceipt.sequence {
            completion(Self.result(.staleIntent, "The paste intent does not refer to Snapik's current package."))
            return
        }
    ...
    /// Snapik intercepted (SPEC-DELTA-2A §1): publish image 1 alone, guarded-send the intent's
