...
/// Port of `src/Snapik.Infrastructure/Persistence/JsonSessionStore.cs`.
///
...
public final class JsonSessionStore: SessionStore {
    ...
    public static func createDefault() -> JsonSessionStore {
        JsonSessionStore(sessionsRoot: SnapikPaths.defaultSessionsDirectory())
    }
    ...
    public func save(_ session: SnapikSession) async throws {
    ...
    public func load(sessionId: SBGuid) async throws -> SnapikSession? {
        ...
        let session = try SnapikJson.decoder.decode(SnapikSession.self, from: data)
        try SessionValidation.validate(session)
        return session
    }
}
