...
public partial class MainWindow : Window, INotifyPropertyChanged
{
    ...
    public MainWindow(LaunchOptions options, bool secondary = false)
    {
        ...
        if (options.Demo && string.IsNullOrWhiteSpace(dataRoot))
            dataRoot = Path.Combine(Path.GetTempPath(), "Snapik", $"demo-{Environment.ProcessId}");
    ...
    {
        if (_prepared is null && !await PrepareAsync()) return;
        using var dialog = new WinForms.FolderBrowserDialog { Description = "Выберите папку для пакета Snapik", UseDescriptionForTitle = true };
        if (dialog.ShowDialog() != WinForms.DialogResult.OK || _prepared is null) return;
        try
        {
            var destination = Path.Combine(dialog.SelectedPath, $"Snapik-{DateTime.Now:yyyyMMdd-HHmmss}");
    ...
    private async void OnClosing(object? sender, CancelEventArgs e)
    {
        ...
        {
            Hide();
            SetStatus("Snapik продолжает работать в области уведомлений.");
        }
    }
