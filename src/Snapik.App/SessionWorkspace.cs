...
public sealed class SessionWorkspace
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var nextSession = SnapikSession.Create(DateTimeOffset.UtcNow);
    ...
    private SnapikSession CreateSnapshot(IEnumerable<CaptureItem> captures, string globalNote, string? profileId)
    {
        var now = DateTimeOffset.UtcNow;
        var frozenCaptures = captures.Select(c => c.ToCore()).ToImmutableArray();
        return new SnapikSession(SessionId, SnapikSession.CurrentSchemaVersion, _createdAtUtc, now, ++_revision,
            globalNote, profileId, frozenCaptures);
    }

    private async Task SaveSnapshotAsync(SnapikSession session, CancellationToken cancellationToken)
    {
    ...
    public static BitmapSource CreateDemoBitmap(int index, int width = 1280, int height = 720, double dpi = 96)
    {
        ...
        using (var dc = visual.RenderOpen())
        {
            ...
            var body = new FormattedText("Проверка Snapik: отдельный снимок и связанная заметка.", System.Globalization.CultureInfo.CurrentUICulture,
