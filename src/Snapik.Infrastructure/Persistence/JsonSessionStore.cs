...
public sealed class JsonSessionStore : ISessionStore
{
    ...
    public static JsonSessionStore CreateDefault()
    {
        ...
        return new JsonSessionStore(Path.Combine(localData, "Snapik", "sessions"));
    }

    public string GetSessionDirectory(Guid sessionId) => Path.Combine(_sessionsRoot, sessionId.ToString("N"));

    public async Task SaveAsync(SnapikSession session, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                await JsonSerializer.SerializeAsync(stream, session, SnapikJson.Options, cancellationToken);
    ...
    public async Task<SnapikSession?> LoadAsync(Guid sessionId, CancellationToken cancellationToken = default)
    {
        ...
        var session = await JsonSerializer.DeserializeAsync<SnapikSession>(stream, SnapikJson.Options, cancellationToken)
