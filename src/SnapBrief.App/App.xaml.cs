    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        if (options.SmokeTest)
        {
            ...
            catch (Exception ex)
            {
                ...
                var root = options.DataDirectory ?? Path.Combine(Path.GetTempPath(), "SnapBrief", "smoke-failure");
...
public static class StartupTrace
{
    public static string GetDataRoot(LaunchOptions options) => options.DataDirectory
        ?? (options.Demo ? Path.Combine(Path.GetTempPath(), "SnapBrief", $"demo-{Environment.ProcessId}") : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "SnapBrief", "sessions"));
...
public sealed record LaunchOptions(bool Demo, bool SmokeTest, string? DataDirectory)
{
    public static LaunchOptions Parse(IReadOnlyList<string> args)
    {
        ...
        data ??= Environment.GetEnvironmentVariable("SNAPBRIEF_DATA_DIR");
