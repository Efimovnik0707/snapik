...
public sealed class ExtendedCommentsTests
{
    ...
    public void Hundreds_of_comments_preserve_identity_links_and_arrow_style_in_json()
    {
        ...
        var session = SnapikSession.Create(DateTimeOffset.UtcNow) with { Captures = [capture] };
        var json = System.Text.Json.JsonSerializer.Serialize(session, SnapikJson.Options);
        var restored = System.Text.Json.JsonSerializer.Deserialize<SnapikSession>(json, SnapikJson.Options)!;
    ...
    public void Note_offset_survives_json_and_stays_null_in_a_session_written_without_it()
    {
        ...
        var beforeTheField = System.Text.Json.JsonSerializer.Deserialize<SnapikSession>(legacy.ToJsonString(), SnapikJson.Options)!;
    ...
    public void Shape_and_fill_survive_json_and_default_for_a_session_written_without_them()
    {
        ...
        written.Remove("fill");
        var beforeTheFields = System.Text.Json.JsonSerializer.Deserialize<SnapikSession>(legacy.ToJsonString(), SnapikJson.Options)!;
    ...
    public void Cropping_keeps_comment_but_clears_a_removed_parent_link()
    {
        ...
        var cropped = Snapik.Core.Editing.CaptureCropper.Crop(capture, new(0, 0, .5, .5), "source/crop.png", 500, 500).CroppedCapture;
