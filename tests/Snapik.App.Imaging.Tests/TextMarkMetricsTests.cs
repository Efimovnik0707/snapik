...
public sealed class TextMarkMetricsTests
{
    ...
    public void A_copy_of_a_mark_keeps_the_pattern_of_its_stroke()
    {
        var dashed = new AnnotationItem
        {
            ...
            LineStyle = Snapik.Core.Models.AnnotationLineStyle.Dashed
        };

        Assert.Equal(Snapik.Core.Models.AnnotationLineStyle.Dashed, dashed.Clone().LineStyle);
        // And the way to the session and back is the same two steps the editor takes.
        var core = dashed.ToCore(800, 600);
        Assert.Equal(Snapik.Core.Models.AnnotationLineStyle.Dashed, core.LineStyle);
        Assert.Equal(Snapik.Core.Models.AnnotationLineStyle.Dashed, AnnotationItem.FromCore(core, 800, 600).LineStyle);
    }
